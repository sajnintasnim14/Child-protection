---
title: "CPA Daily Monitoring Report(caregiver)"
author: "MH Khan"
...
data_path <- "D:\\mh1\\REACH\\cpp\\CPP\\04 Data\\02_caregiver\\01_data_collection/"
# dropbox_hh_path <- paste0(dropbox_path,"daily_data\\",str_replace_all(ymd(day_to_run),"-","_"),"_hh_Data.csv")
date_log_path <- "D:\\mh1\\REACH\\cpp\\CPP\\Output\\date_log/caregiver/date_logger.csv"
...
target_points <- st_read("Output/sampling/kml/cat//round1/merged_care.kml")
...
date_log_track <- paste0("D:\\mh1\\REACH\\cpp\\CPP\\Output\\date_log/caregiver/",str_replace_all(day_to_run,"-","_"),"_date_logger.csv") 
...
hh_to_output_path <- "C:\\Users\\MEHEDI\\Dropbox\\REACH_BGD\\REACH\\Ongoing\\70DYL - UNICEF (Child Protection)\\04 Data\\02_caregiver\\01_data_collection\\daily_data/"
...
sample_size <- read.csv("Inputs/Sampling/cat/kmz_sample_request_template_round1.csv")
...
path_unzip <- "Inputs\\caregiver\\98_temp"
audit_zip_dir<-"D:\\mh1\\REACH\\cpp\\CPP\\04 Data\\02_caregiver\\01_data_collection\\01_audit"
audit_zipfile <-paste0(audit_zip_dir,"/atRNMxM729fHnbnSwYAsC4_",str_replace_all(day_to_run,"-","_"),".zip")
copy_zip_to<-paste0("D:\\mh1\\REACH\\cpp\\CPP\\Inputs\\caregiver\\01_audit/",day_to_run,".zip")
audit_node<-"/atRNMxM729fHnbnSwYAsC4/"
...
for (i in 1: length(audit_yes)){
  ...
  end_question<-d %>% filter(node==paste0(audit_node,"survey_consent/end_interview")& !is.na(node)) %>% 
    select(start)
...
for (i in 1: length(audit_yes)){
  d<-audit_yes[[i]]
  start_question<- d %>% filter(node==paste0(audit_node,"survey_consent/harmthreats/witness_threats")& !is.na(event)) %>% 
    select(start) 
  start_question<-min(start_question$start)
  end_question<-d %>% filter(node==paste0(audit_node,"survey_consent/harmthreats/witness_threats")& !is.na(node)) %>% 
    select(end)
  end_question<-max(end_question$end)
  duration_ms<-end_question-start_question
  duration_secs<-duration_ms/1000
...
```{r,include= TRUE, warning = FALSE, message=FALSE}
#total col 39+5=44
col_name_enu_si <- c("work_chores", "work_shelter",
"work_phys_chores", "work_sibs", "work_outside", "work_dang",
"work_constr", "work_stop_school", "marr_par", "marr_dowry",
"marr_burden", "marr_safe", "marr_drop_school_girl", "marr_drop_school_boy",
"marr_old_man", "marr_mom", "vio_fath", "vio_moth", "vio_discipline",
"vio_husb", "vio_wife", "vio_teach", "vio_solve_prob",
"negl_misbehave", "negl_stress", "negl_needs","threats_1","threats_2",                                         "threats_3","threats_4","threats_5","threats_6","threats_7",
"threats_8","threats_9","threats_10","threats_11","threats_12",
"threats_13")


col_name_enu_multiple <- c("threats_30.dont_know","report_child_work.dont_know",
                           "report_child_vio_home.dont_know", "report_child_vio_teach.dont_know",
                           "report_child_vio_sex.dont_know")


hh_spatial_corrected_yes_consent_df2 <- hh_spatial_corrected_yes_consent %>% as.data.frame()

 hh_spatial_corrected_yes_consent_df2$dontknow_prefer_not_to_ans_si <- rowSums(hh_spatial_corrected_yes_consent_df2[col_name_enu_si] =="dont_know",na.rm = T)

 hh_spatial_corrected_yes_consent_df2$dontknow_prefer_not_to_ans_multiple <- rowSums(hh_spatial_corrected_yes_consent_df2[col_name_enu_multiple] =="1",na.rm = T)


 hh_spatial_corrected_yes_consent_df2$rowsum_dont_know_enu = ( hh_spatial_corrected_yes_consent_df2$dontknow_prefer_not_to_ans_multiple + hh_spatial_corrected_yes_consent_df2$dontknow_prefer_not_to_ans_si)

 enu_records_dont_know <- hh_spatial_corrected_yes_consent_df2 %>% dplyr::filter(rowsum_dont_know_enu > 22) %>% select(reported_date, enumerator_id, X_uuid, New_Camp_N)
...
dropbox_aux<-"C:\\Users\\MEHEDI\\Dropbox\\REACH_BGD\\REACH\\Ongoing\\70DYL - UNICEF (Child Protection)\\04 Data\\02_caregiver/01_data_collection/aux_outputs/"
...
# write.csv( latrine_na,paste0(dropbox_aux_na_path, "_NA_response_latrine.csv"))
}
...
### Agree/disagree
``` {r eval=TRUE, echo=FALSE, results='asis', warning=FALSE, messages=FALSE}
col_names <- c("work_chores","work_shelter","work_phys_chores","work_sibs","work_outside","work_dang","work_constr","work_stop_school","marr_par","marr_dowry","marr_burden","marr_safe","marr_drop_school_girl","marr_drop_school_boy","marr_old_man","marr_mom","vio_fath","vio_moth","vio_discipline","vio_husb","vio_wife","vio_teach","vio_solve_prob","negl_misbehave","negl_stress","negl_needs")
